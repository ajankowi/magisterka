...
def generate_style_mix(
    ...
    for row_seed in row_seeds:
        for col_seed in col_seeds:
            ...
            #print(image.size())
            image=image.view(512,512)
            #print("later:",image.size())
            #image_dict[(row_seed, col_seed)] = image[0].cpu().numpy()
            #print(np.asarray(image.cpu().numpy()))
            
            image_dict[(row_seed, col_seed)] = image.cpu().numpy()
    ...
    for (row_seed, col_seed), image in image_dict.items():
        ...
        PIL.Image.fromarray(image,"L").save(f'{outdir}/{row_seed}-{col_seed}.png')
    ...
    for row_idx, row_seed in enumerate([0] + row_seeds):
        for col_idx, col_seed in enumerate([0] + col_seeds):
            ...
            image_dict[key].shape    
            canvas.paste(PIL.Image.fromarray(image_dict[key].reshape(512,512), 'L'), (W * col_idx, H * row_idx))
